...
Because of insertion sort’s low hidden constant value, however, it usually outperforms more advanced algorithms such as quick sort or merge sort on smaller lists.
Its best case run time is O(n), or linear, if the input array is already sorted.
LOGIC
Insertion sort is  adaptive: it works well with arrays that are already partially or fully sorted: it reduces its run time to O(nk), where each element in the list is no more than k elements away from it’s sorted position.
